...
- Responsive output using Bootstrap (CSS, JS, and default favicon bundled automatically by `krems --build`)
...
1.  **Create Markdown Files:** You create your `.md` files and any custom static assets (like non-default images in an `images/` folder, or custom JavaScript in `js/`) directly in the root of your GitHub repository. Core CSS (Bootstrap), core JS (Bootstrap JS), and a default favicon are handled automatically by `krems`. Subfolders for organizing markdown content are also supported (e.g., `my-articles/article1.md`).
...
1.  **`krems --build`**  
    ...
    - Automatically generating necessary CSS (Bootstrap), JS (Bootstrap JS), and a default favicon into `docs/css/`, `docs/js/`, and `docs/images/` respectively.
    - Copying any additional user-provided assets from `images/` and `js/` (if they exist at the root) into `docs/`. User-provided files with the same name (e.g., `js/bootstrap.js`, `images/favicon.ico`) will overwrite the defaults.
...
- `articles/` (a folder for your articles)
  ...
  - `my-first-article.md`
  - `another-topic.md`
- `images/` (for your custom images, e.g., `photo.jpg`)
- `js/` (for your custom JavaScript, e.g., `custom.js`)
- `config.yaml`

Core CSS (Bootstrap), core JS (Bootstrap JS), and a default favicon are automatically included by `krems` during the build process. You do not need to create `css/`, `js/bootstrap.js`, or `images/favicon.ico` at the root for these defaults. If you wish to add your *own* custom CSS, JS, or override the default favicon, you can place them in the respective root folders (`css/`, `js/`, `images/`). User-provided files will take precedence if they share the same name and path as the defaults generated by `krems`.
...
-   **Markdown and Asset Location:** `krems` (from v0.2.0 or the version incorporating these simplified workflow changes) no longer looks for content within a `markdown/` subdirectory.
    ...
    *   User-provided static assets (like custom images in `images/` or custom scripts in `js/`) should also be at the root level in their respective folders. Core CSS, core JS (Bootstrap), and a default favicon are handled internally by `krems`.
